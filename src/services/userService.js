    ...
    try {
        ...
        for (const amount of [100,500]) {
            genTeamCounts[amount] = {
                ...
                gen2_3: await countActiveUsers(genTeam[1], amount) + await countActiveUsers(genTeam[2], amount) + await countActiveUsers(genTeam[3], amount) + await countActiveUsers(genTeam[4], amount) + await countActiveUsers(genTeam[5], amount)
            };
        }
...
async function myLevelTeamCount(userId, level = 5) {
    try {
    ...
    try {
        ...
        const transporter = nodemailer.createTransport({
            ...
            auth: {
                ...
                pass: "E;W)cViIDp^SyVMo", // Your email password
            },
        });
        const mailOptions = {
